...
description: Diagnose Swift concurrency issues, refactor callback-based code to async/await, and guide migration when working with tasks, actors, @MainActor, Sendable, data races, thread safety, or concurrency diagnostics. Inspect the project's Swift version and enabled features before recommending version-specific APIs.
---
# Swift Concurrency
...
4. Confirm whether the code is UI-bound or intended to run off the main actor. When spawning unstructured
   tasks, inspect the synchronous prefix (everything before the first `await`). Use `@concurrent` only when
   the compiler and project enable its Swift 6.2-era semantics; otherwise preserve the project's supported
   isolation model and move CPU work behind an appropriate actor, service, or carefully justified detached
   task. A trivial non-main line followed by main-actor work is not a reason to hop isolation. For delayed
   retries, timers, and backoff tasks, separate waiting from UI mutation.
...
If any of these are unknown, inspect the project or ask the developer to confirm them before giving migration-sensitive guidance. Never infer settings from the toolchain version alone.
...
- **Background work**: when work must hop off caller isolation and the project supports Swift 6.2
  `@concurrent`, use it deliberately. On older language modes, preserve the existing isolation contract and
  use a nonisolated service, dedicated actor, or narrowly justified `Task.detached` with Sendable inputs.
  Network suspension alone does not require forcing a thread hop.
...
Task {
    ...
    updateUI(with: data) // In a @MainActor context, the task inherits UI isolation.
}
```
...
Match a `Task`'s entry isolation to its synchronous prefix (everything from `{` to the first `await`) and
the active Swift language features.

- If anything in that prefix needs `@MainActor`, keep the inherited `@MainActor` start.
- If nothing in that prefix needs `@MainActor` and the project supports Swift 6.2 `@concurrent`, it may start
  the task away from inherited isolation; hop back only for UI-owned mutation.
- On older language modes, call a supported nonisolated service or dedicated actor. Use `Task.detached` only
  for a documented boundary with Sendable inputs and explicit cancellation/lifetime handling.

```swift
// Ordinary Task inherits the surrounding actor context.
Task {
    await service.load()
}

// ✅ Swift 6.2+ when the project enables @concurrent semantics:
// start off the main actor, then hop back only for UI work
Task { @concurrent in
