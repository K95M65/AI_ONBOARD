...
8. **Adopt compatible concurrency features deliberately** - inspect the active Swift mode and upcoming
   features before changing them
9. **Choose default isolation from the application architecture** - preserve existing behavior and migration
   constraints
10. **Don't force thread switching** - let Swift optimize

## Migration Strategy

### For compatible new projects

1. Inspect the toolchain, target defaults, dependencies, and application architecture.
2. Consider `@MainActor` default isolation when the product is predominantly UI-bound and the toolchain
   supports it.
3. Enable `NonisolatedNonsendingByDefault` only when its execution semantics match the project.
4. Use `@concurrent` for explicit background work only when that feature is enabled and supported.
