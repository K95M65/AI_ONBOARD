...
The example below uses `onScrollGeometryChange`, which requires iOS/iPadOS 18 or macOS 15, and
`onGeometryChange`, which is available from iOS/iPadOS 16 or macOS 13. Confirm the target platform and
deployment version before using it.
...
Keep the same normalized `progress` model, but collect geometry with APIs supported by the project:

1. Give the scroll container a named coordinate space.
2. Put a lightweight `GeometryReader` behind the primary or secondary section.
3. Send its frame or offset through a `PreferenceKey`.
4. Update `revealProgress` only when the derived value changes.
5. Use the same preference pattern to measure the secondary height.

Gate the modern implementation with `#available` and keep the fallback in a separate view or modifier so
both branches remain testable. `onScrollTargetVisibilityChange` is also a modern scroll API; use the
preference-derived progress threshold when it is unavailable.
